...
if (workerFlag !== -1) {
  const lo = Number(process.argv[workerFlag + 1])
  const hi = Number(process.argv[workerFlag + 2])
...
const results = await Promise.all(
  Array.from({ length: jobs }, (_, i) => {
    ...
    return new Promise<CuratedLevel[]>((res, rej) => {
      const child = fork(self, ['--worker', String(lo), String(hi)], { silent: false })
...
const pack = all.slice(0, keep).sort((a, b) => a.metrics.skills - b.metrics.skills || a.score - b.score)
...
writeFileSync(
  outPath,
  `${JSON.stringify(
    { generated: candidates, kept: pack.length, levels: pack.map((l) => ({ seed: l.seed, spec: l.spec, metrics: l.metrics })) },
    null,
    1,
  )}\n`,
)
...
const avg = (f: (l: CuratedLevel) => number) => all.reduce((s, l) => s + f(l), 0) / Math.max(1, all.length)
console.log(
  `accepted ${all.length}/${candidates} (${((all.length / candidates) * 100).toFixed(0)}%) in ${secs.toFixed(1)}s ` +
    `— ${(candidates / secs).toFixed(0)} candidates/sec`,
)
console.log(
  `pool avg: skills=${avg((l) => l.metrics.skills).toFixed(1)} alts=${avg((l) => l.metrics.alternatives).toFixed(1)} ` +
